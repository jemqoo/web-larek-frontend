...
3. Класс Component - абстрактный класс, родительский класс для всех классов отображения, имеет возможность:
...
### Компоненты представления состоят из Basket, Form, Modal, Success, Card, Order, Page:
...
3) Класс Modal данный компонент отвечает за работу модальных окон и вывод в них контента, в данном компоненте содержатся следующие поля:
...
5) Класс Card данный компонент предназначен вывода данных о товаре на страницу,
...
Методы:
    ...
    "set/get category(value: string)" - для установки/получения category в соответвующие элементы на страницы;
    "set image(value: string)" - для установки кратинки;
    "set itemIndex(value: number)" - сеттер для индекса;
    "set description(value: string | string[])" - сеттер для описания;

6) Класс OrderForm данный компонент предназначен для работы с формой "Order", в данном компоненте содержатся следующие поля:
    button - HTML-элемент для открытия следующего модального окна;
...
7) Класс ContactForm данный компонент предназначен для работы с формой "contacts",имеет следующие поля:
...
8) Класс Page данный компонент предназначен вывода основных данных на страницу, компонент имеет следующие поля:
...
Класс LarekAPI расширяет базовый класс Api, данный компонент  предоставляет методы для работы с API, обеспечивает функциональность для получения списка карточек, информации о продуктах и оформления заказов.
